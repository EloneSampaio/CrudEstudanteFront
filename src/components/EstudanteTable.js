...
`;
  ...
  const fetchData = async () => {
    const result = await axios("http://31.220.31.176:8000/api/estudantes");
    setData(result.data);
  };

  const botaoAddStudent = async student => {
    await axios.post("http://31.220.31.176:8000/api/estudantes", student);
    fetchData();
    setFormVisible(false);
  };

  const botaoUpdateStudent = async student => {
    await axios.put(`http://31.220.31.176:8000/api/estudantes/${selectedStudent.id}`, student);
    fetchData();
  ...
  const botaoDeleteStudent = async id => {
    await axios.delete(`http://31.220.31.176:8000/api/estudantes/${id}`);
    fetchData();
  };
  ...
  const columns = React.useMemo(
    () => [
      ...
      },
      {
        Header: "Foto",
        accessor: "foto",
        Cell: ({ value }) => <img src={`http://31.220.31.176:8000/${value}`} alt="Foto" width="50" />,
      },
      {
        Header: "actions"
      }
